 ...
 * @return Page 
 */
Page BufferManager::getPage(string entityName, int pageIndex,int entityType)
{
    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+entityName + "_Page" + to_string(pageIndex);
    if (this->inPool(pageName))
        return this->getFromPool(pageName);
    else
        if(entityType == 0)
        return this->insertIntoPool(entityName, pageIndex,0);
        else if (entityType == 1)
        return this->insertIntoPool(entityName,pageIndex,1);
}
 ...
 * @return Page 
 */
Page BufferManager::insertIntoPool(string entityName, int pageIndex,int entityType)
{
    logger.log("BufferManager::insertIntoPool");
    Page page(entityName, pageIndex,entityType);
...
void BufferManager::writePage(string tableName, int pageIndex, vector<int> elements, int elementCount)
{
    logger.log("BufferManager::writeMatrixPage");
    Page page(tableName, pageIndex,elements,elementCount);
